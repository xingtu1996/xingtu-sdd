***REMOVED*** specs-engine — Spec 驱动开发方法论引擎
...
***REMOVED******REMOVED*** 为什么有它（对比 2026 主流方案）
...
***REMOVED******REMOVED*** 安装与使用（三步）

```bash
***REMOVED*** 0. 安装：把 specs-engine 放到用户级 skills（所有项目生效；改一处全局升级）
cp -r specs-engine ~/.claude/skills/     ***REMOVED*** 或 symlink

***REMOVED*** 1. 每个新任务（AI 也可自动触发：说"按 specs 标准推进"）
bash ~/.claude/skills/specs-engine/scripts/scaffold.sh new .claude/specs {TICKET}-{简述} [--depth light|standard|heavy]

***REMOVED*** 2. 按五阶段推进，05_validator 写完自查
...
```
specs-engine/
├── README.md                ***REMOVED*** 本文件
├── SKILL.md                 ***REMOVED*** 入口：触发词 + 意图路由表 + 深度裁减表（SSOT）
├── references/
│   ├── core.md              ***REMOVED*** 方法论正文：三铁律 + 五阶段 + 文件规范 + 门禁
│   └── orchestration.md     ***REMOVED*** 多 Spec 并行（仅并行场景读）
├── templates/               ***REMOVED*** 编号制 8 骨架：00_README / laws / 01_analysis ~ 05_validator / execution-log
├── scripts/
│   ├── scaffold.sh          ***REMOVED*** new：建 spec 骨架 + 索引加行（模板只从全局 cp，不产项目快照）
│   └── validator-check.sh   ***REMOVED*** 静态检查 validator 填充完整性（占位符残留真门禁）
└── examples/
    └── demo-bugfix/         ***REMOVED*** 一份填好的轻量 spec（教程锚点）
```

***REMOVED******REMOVED*** 为什么「可迁移、不分叉」
